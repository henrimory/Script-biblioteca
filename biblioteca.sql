...
create table tbl_editora(
    ...
    email varchar(100) default "Editora sem email"
);
...
create table tbl_cliente(
    ...
    email varchar(45) default "Cliente sem email"
);

create table tbl_bibliotecario(
	codigo_bibliotecario int primary key auto_increment,
    nome_bibliotecario varchar(45) not null,
    nasc_bibliotecario date not null,
    telefone varchar(45) not null,                      
    FK_Biblio_responsavel int,
    constraint FK_Biblio_responsavel
    FOREIGN KEY (FK_Biblio_responsavel) REFERENCES tbl_bibliotecario (codigo_bibliotecario)
);

create table tbl_emprestimo(
	codigo_emprestimo int(11) primary key not null auto_increment,
    retirada datetime not null,
    devolucao datetime not null,
    FK_Bibliotecario int not null,                   
    FK_Cliente int not null,
	constraint FK_Biblioteca FOREIGN KEY (FK_Bibliotecario) REFERENCES tbl_bibliotecario (codigo_bibliotecario),
...
create table livro_emprestimo(
	...
	constraint FOREIGN KEY (FK_Livro) REFERENCES tbl_livro (codigo_livro),           
	constraint FOREIGN KEY (FK_Emprestimo) REFERENCES tbl_emprestimo (codigo_emprestimo)
);
...
create table autor_livro(
    ...
    constraint FK_Livro foreign key (FK_Livro) references tbl_Livro (codigo_livro)       
);
...
-- Insert's tbl_Livro
-- -----------------------------------------------------
INSERT INTO tbl_Livro (nome_livro, data_publicacao, paginas, valor_unitario, FK_Categoria, FK_Editora) VALUES
...
INSERT INTO Livro_Emprestimo VALUES
    ...
    INSERT INTO tbl_Autor (nome_autor, nasc_autor, nacionalidade) VALUES 
	('t5ete','1828/09/09','Russia');

    
---------------------------------------------------------------------------------
#primeiro JOIN 
select tbl_livro.nome_livro, tbl_categoria.nome_categoria from tbl_livro
join tbl_categoria on tbl_categoria.codigo_categoria = tbl_livro.fk_categoria;
---------------------------------------------------------------------------------------------------    
#exibir o nome do livro, o nome da categoria e o nome da editora.
select tbl_livro.nome_livro, tbl_categoria.nome_categoria, tbl_editora.nome_editora from tbl_livro
join tbl_categoria on tbl_categoria.codigo_categoria = tbl_livro.fk_categoria
join tbl_editora on tbl_editora.codigo_editora = tbl_livro.fk_editora;   

----------------------------------------------------------------------------------------------
select tbl_livro.nome_livro as Os_livros, tbl_editora.nome_editora as Pertencem_a_Editora from tbl_livro
join tbl_editora on tbl_editora.codigo_editora = tbl_livro.FK_Editora where nome_editora = "Somos Educação" ;
-----------------------------------------------------------------------------------------------------------------    
select  tbl_autor.nome_autor as O_Autor, tbl_categoria.nome_categoria as Escreveu_a_Categoria from tbl_livro    
join tbl_autor on tbl_autor.codigo_autor = tbl_livro.fk_categoria
join tbl_categoria on tbl_categoria.codigo_categoria = tbl_livro.fk_categoria where nome_categoria = "Ficção";

select nome_autor, nome_livro, nome_categoria from tbl_autor
join autor_livro ON codigo_autor = fk_autor
join tbl_livro on fk_livro = codigo_livro
join tbl_categoria on fk_categoria = codigo_categoria
where nome_categoria = "Ficção";
    ------------------------------------------------------------------------------------------------------------------
    
select tbl_livro.nome_livro as Esses_Livros, tbl_autor.nome_autor as Pertencem_aos_Autores, tbl_autor.nacionalidade as Nação from tbl_livro
join tbl_autor on tbl_autor.codigo_autor = tbl_livro.fk_categoria
join tbl_categoria on tbl_categoria.codigo_categoria = tbl_livro.fk_categoria where nacionalidade = "Inglaterra";

select tbl_livro.nome_livro, tbl_autor.nome_autor, tbl_autor.nacionalidade from autor_livro
...
select nome_livro, nome_categoria from tbl_categoria left join tbl_livro on
fk_categoria = codigo_categoria;

#exemplo de right join
select nome_livro, nome_categoria from tbl_categoria right join tbl_livro on
fk_categoria = codigo_categoria;

#exemplo full join

select nome_livro, nome_categoria from tbl_categoria
left join tbl_livro on fk_categoria = codigo_categoria;
...
select tbl_autor.codigo_autor,tbl_autor.nome_autor,tbl_autor.nasc_autor,tbl_autor.nacionalidade from tbl_autor
where nome_autor in ('JK ROWLING', 'STEPHAN KING');
select * from tbl_autor where nome_autor in ('JK ROWLING', 'STEPHAN KING');
--------------------------------------------------------------------------------------------------
# 2 - SELECIONAR O NOME, AS PAGINAS E A CATEGORIA DE TODOS OS LIVROS DAS CATEGORIAS "CONTOS", "POESIA" E "AVENTURA"
select tbl_livro.nome_livro, tbl_livro.paginas, tbl_categoria.nome_categoria from tbl_livro
join tbl_categoria on tbl_categoria.codigo_categoria = tbl_livro.fk_categoria where nome_categoria in ('Contos','Poesia','Aventura');

select a.nome_livro, a.paginas, b.nome_categoria from tbl_livro a
join tbl_categoria b on a.fk_categoria = b.codigo_categoria = fk_categoria
where nome_categoria in('Contos','Poesia','Aventura');
----------------------------------------------------------------------------------------------------------------------------
# 3 - EXIBIR QUAL CATEGORIA DE LIVROS, NAO TEM NENHUM LIVRO VINCULADO A ELA.
select a.nome_categoria from tbl_categoria a left join tbl_livro b on a.codigo_categoria = b.fk_categoria
...
select a.nome_livro, b.nome_categoria, c.nome_editora from tbl_livro a
...
select tbl_autor.nome_autor, count(fk_autor) total from tbl_autor
join autor_livro on codigo_autor = fk_autor
join tbl_livro on codigo_livro = fk_livro
join tbl_categoria on codigo_categoria = fk_categoria
where nome_categoria = 'Romance' group by nome_autor order by total desc;

select nome_autor, count(*) Total from autor_livro
join tbl_livro on fk_livro = codigo_livro
join tbl_categoria on fk_categoria = codigo_categoria
join tbl_autor on fk_autor = codigo_autor
...
select tbl_autor.nome_autor, tbl_livro.nome_livro from autor_livro
join tbl_autor on fk_autor = codigo_autor
join tbl_livro on fk_livro = codigo_livro;  

select a.nome_autor, b.nome_livro from autor_livro c
join tbl_autor a on c.fk_autor = a.codigo_autor
join tbl_livro b on c.fk_livro = b.codigo_livro;
--------------------------------------------------------------------------------
...
select tbl_autor.nome_autor, count(fk_livro) total_livros from tbl_autor
join autor_livro on codigo_autor = fk_autor
join tbl_livro on codigo_livro = fk_livro    
group by nome_autor order by total_livros desc;

select a.nome_autor, count(*) total_livros from autor_livro b 
join tbl_autor a on b.fk_autor = a.codigo_autor
group by fk_autor
order by total_livros desc;

#8-EXIBIR APENAS O NOME, A SUA CATEGORIA E VALOR_UNITARIO PARA TODOS OS LIVROS QUE O VALOR UNITARIO FOR SUPERIOR A MEDIA DOS PREÇOS.
#ORDENAR A BUSCA DO MENOR VALOR PARA O MAIOR. # DICA: UTILIZE SUBQUERY E ORDER BY

select tbl_Livro.nome_livro, tbl_categoria.nome_categoria, tbl_livro.valor_unitario from tbl_livro
join tbl_categoria on codigo_categoria = fk_categoria
where valor_unitario > (select avg(valor_unitario)from tbl_livro) order by valor_unitario asc;

select a.nome_livro, b.nome_categoria, a.valor_unitario from tbl_livro a 
join tbl_categoria  b on a.fk_categoria = b.codigo_categoria
where valor_unitario > (select avg(valor_unitario)from tbl_livro) order by valor_unitario ; # por padrão ele utiliza a propriedade asc


#9- EXIBIR O NOME DO LIVRO, A QUANTIDADE DE PAGINAS E O NOME DA CATEGORIA PARA TODOS OS LIVROS QUE O NUMERO DE PAGINAS
# SEJA MENOR QUE A MEDIA DAS PAGINAS. ORDERNAR A BUSCA DO MAIOR PARA O MENOR VALOR DA COLUNA PAGINAS

select tbl_livro.nome_livro, paginas, tbl_categoria.nome_categoria from tbl_livro
join tbl_categoria on codigo_categoria = fk_categoria 
where paginas < (select avg(paginas)from tbl_livro) order by paginas desc;

select a.nome_livro, a.paginas, b.nome_categoria from tbl_livro a 
join tbl_categoria b on a.fk_categoria = b.codigo_categoria
where paginas < (select avg(paginas) from tbl_livro)
order by paginas desc;


#10-EXIBIR O NOME DE TODOS OS LIVROS E SUAS EDITORAS PARA AQUELES LIVROS QUE FORAM EMPRESTADOS NOS ULTIMOS 18 MESES. 
#MOSTRAR OS RESULTADOS ORGANIZADOS POR DATA DE EMPRESTIMO
select tbl_livro.nome_livro, tbl_editora.nome_editora, tbl_emprestimo.retirada from tbl_livro
join livro_emprestimo on fk_livro = codigo_livro
join tbl_emprestimo on codigo_emprestimo = fk_emprestimo
join tbl_editora on codigo_editora = fk_editora
where retirada > curdate() - interval 18 month order by retirada;

select a.nome_livro, b.nome_editora, c.retirada from livro_emprestimo d
join tbl_livro a on d.fk_livro = a.codigo_livro
join tbl_editora b on a.fk_editora = b.codigo_editora
join tbl_emprestimo c on d.fk_emprestimo = c.codigo_emprestimo
where c.retirada > (now() - interval 18 month) order by retirada;

#WHERE DATE_SUB(NOW(), INTERVAL 18 MONTH) <= tbl_emprestimo.retirada
------------------------------------------------------------------------------------------------

create view emprestimosrealizados as
	select a.nome_livro, b.nome_editora, c.retirada from livro_emprestimo d
	join tbl_livro a on d.fk_livro = a.codigo_livro
	join tbl_editora b on a.fk_editora = b.codigo_editora
	join tbl_emprestimo c on d.fk_emprestimo = c.codigo_emprestimo
...
select * from emprestimosrealizados;
#drop view emprestimosrealizados;
